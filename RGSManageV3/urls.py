...
urlpatterns = [
    url(r'^v3/admin/', admin.site.urls),
    url(r'^v3/login/$', obtain_jwt_token),
    url(r'^v3/', include(router.urls)),
    # url(r'^v3/docs/', include_docs_urls(title=u"库管系统API",authentication_classes=[JSONWebTokenAuthentication, SessionAuthentication])),
    url(r'^v3/docs/', include_docs_urls(title=u"库管系统API")),
    url(r'^v3/media/(?P<path>.*)$', serve, {"document_root": settings.MEDIA_ROOT}),
    url(r'^v3/api-auth/', include('rest_framework.urls', namespace='rest_framework')),
]
